...
import robocam.overlay.imageassets as imga
import robocam.servos.pid as pid
from robocam.helpers import utilities as util
...
def camera_process(shared_data_object):
    ...
    for i in range(args.faces):
        pie = imga.ImageAsset(pie_folder, position=(500, 500))
        pie.coords = shared.bbox_coords[i, :] # reference a line in teh shared array
        Pies.append(pie)
    ...
    while True:
        ...
        for i in range(shared.n_faces.value):
            center = util.bbox_to_center(shared.bbox_coords[i, :])
            Pies[i].write(capture.frame, position=center)
